...
** PRELIMINARIES / INITIALIZATION OF VARIABLES:
      ...
      do i=0,Nx+1                                              !! Initialize the wavefunction to some initial
	    ...
	    psi2(i,j)=dcmplx(0.d0,0.d0) 
            ...
            x=-xmax+dble(i-1)*dx
	    psi(i,j)=dcmplx(Exp(-x**2),0.d0)                   !! Set that initial wave function
            psi2(i,j)=dcmplx(Exp(-x**2),0.d0)
	  end if                           
      ...
      do i=1,Nx                                             !! Loop over x-indices
       ...
       do j=1,Ny                                            !! Loop over y-indices.
        ...
        !!V(i,j)=-100.0d0*(atan(1.0d0*(x+150.0d0*t - 8.0d0)) + pi/2.0d0)          
          V(i,j)=100.0d0*Exp(-(x+45.0d0*t - 6.0d0)**2)
     ...
     $     psi(i-1,j))/(2.d0*dx)))
      ...
      do i=1,Nx
        x=-xmax+dble(i-1)*dx
          j=((Ny+1)/2)
          y=-ymax+dble(j-1)*dy
          write(12,'(4e14.6)')real(x),
     &real(datan2(dimag(psi(i,j)),dreal(psi(i,j)))),  !! making these smaller for viewing convenience
     &real(zabs(psi(i,j))**2),
     &real(Exp(-(x+45.0d0*t - 6.0d0)**2))
