...
** PRELIMINARIES / INITIALIZATION OF VARIABLES:
      ...
      dt=dcmplx(0.d0,-dt_real)                                 !! Set the time step:
...
** REAL TIME OPERATIONS:
        ...
        if(dabs(dreal(dt)).gt.1.d-10)then                       !! If propagating in real time, perform the following tasks:
          ...
          if(itime2.eq.0.or.mod(itime2,200).eq.0)then           !! If the first real time step or every subsequent 200 steps....
            call get_E(psi,V,E)                                 !! Determine energy. 
            call get_N(psi,N)                                   !! Determine particle number.
            write(*,*)'t=',real(t),'N=',real(N),'E=',real(E)    !! Write parameters to screen.
            write(20,*)real(t),real(N),real(E)                  !! Write parameters to file 'main.dat'.
          end if

          if(itime2.eq.0.or.mod(itime2,80).eq.0)then           !! If the first real time step or every subsequent 500 steps....
...
!! time-dependent, in which it should be called every real-time step.  
      ...
      do i=1,Nx                                             !! Loop over x-indices
       ...
       do j=1,Ny                                            !! Loop over y-indices.
         ...
         if(t.lt.dt_real)then			            !! Example: for t=0 we impose a harmonic trap
           V(i,j)=0.5*(x**2+y**2)                   !! offset in x; then for t>0 we move the trap to the centre.
         else
           V(i,j)=0.5*(x**2+y**2)
         endif
