...
*************************************** DEFINITION OF VARIABLES***************************************************
      ...
      double precision    Force,R,P1,phase                     !! force, position,momentum, and phase
      ...
      double precision    N,N_old,N_diff                       !! Variables for the particle number N, 
      ...
      complex*16          psi2(0:Nx+1,0:Ny+1)                  !! Wavefunction used for holding values
...
** PRELIMINARIES / INITIALIZATION OF VARIABLES:
      ...
      do i=0,Nx+1                                              !! Initialize the wavefunction to some initial
        do j=0,Ny+1  					       !! profile, here a homogeneous BEC
          if(j.eq.0.or.j.eq.Ny+1.or.i.eq.0.or.i.eq.Nx+1)then   !! The outer points stay fixed during the whole sim so
            psi(i,j)=dcmplx(1.d0,0.d0)                         !! set to 1 for an infinite system or 0 for a trapped system
          else
            psi(i,j)=dcmplx(1.d0,0.d0)
          end if                           
        end do                                                
      end do
      ...
      open(12,file='2dphase.dat')
      open(20,file='2dmain.dat')
      open(13,file='2dcofm.dat')                                 !! Added an extra center of mass file
      open(14,file='2dmomenta.dat') 
...
** TIME-ITERATION LOOP:
      ...
      do itime=1,Nt
        ...
        call get_RHS_x(dt,psi,U,RHS_x)                          !! Determines the RHS of the x-direction time evo equation.
        ...
        do k=1,2                                                !! Iterative step to account for time-dependent Hamiltonian in GPE
          call solve_x(dt,RHS_x,U,psi)                          !! Solve the time evo equation in x to a get an updated psi
...
** REAL TIME OPERATIONS:
        ...
        if(dabs(dreal(dt)).gt.1.d-10)then                       !! If propagating in real time, perform the following tasks:
          ...
          itime2=itime                                          !! Define new time index for real time propagation

          if(itime2.eq.0.or.mod(itime2,20000).eq.0)then           !! If the first real time step or every subsequent 200 steps....
            call get_E(psi,V,E)                                 !! Determine energy. 
            call get_N(psi,N)                                   !! Determine particle number.
            call get_X(psi,R,xmax,ymax)                         !! Get the position  
	    call get_phase(psi,phase,N) 			!! Get the phase difference			
	    call get_Force(psi,psi2,Force,xmax)                 !! Find the force
            call get_P1(psi,P1,xmax)                            !! Determine momentum
	   write(*,*)'t=',real(t),'X=',real(R),                 !! Write out on screen
     &       'P=',real(P1)/real(N)                              !! Write parameters to screen.
            write(20,*)real(t),real(N),real(E)                  !! Write parameters to file 'main.dat'.
          end if

          if(itime2.eq.0.or.mod(itime2,10000).eq.0)then            !! If the first real time step or every subsequent 500 steps....
            call get_P1(psi,P1,xmax) 
            call get_X(psi,R,xmax,ymax)
            call get_N(psi,N)
	    call get_phase(psi,phase,N) 
	    call movie(xmax,ymax,iframe,psi,t,P1,Force,N,X)     !! Call the routine to genenerate movie frames
	    write(14,*)real(t),real(P1)/real(N)                 !! This makes the center of mass plot
            write(13,*)real(t),real(R)/real(N) 
	    write(12,*)real(t),real(phase)
	  end if
        end if
        ...
        do i=1,Nx
          do j=1,Ny
          psi2(i,j)=psi(i,j)                                    !! Update psi2 for the next time  
          end do
        end do 	
      ...
      do j=0,Ny+1  
        psi(0,j)= psi(Nx,j)                                     !! This is for periodic boundary conditions
        psi(Nx+1,j)= psi(1,j)
      end do
      do i=1,Nx  
        psi(i,0)= psi(i,Ny)                                     !! This is for periodic boundary conditions
        psi(i,Ny+1)= psi(i,1)
      end do
...
!! time-dependent, in which it should be called every real-time step.  
      ...
      integer           i,j
      double precision  V(Nx,Ny),x,y,xmax,ymax,t,c1,c2,c3
      parameter         (pi=3.1415926535897932d0)
      
      open(25,file='potential.dat')                         !! Open a file to write the potential to.
      c2=(0.01d0)*2.1d0*1.12d0
      do i=1,Nx                                             !! Loop over x-indices
       ...
       do j=1,Ny                                            !! Loop over y-indices.
         ...
         c2=0.1566d0*Erf(1.44d0*y)
         c3=0.25d0*y*Exp(-2*y**2)
         if(t.gt.1.d0)then			            !! Example: for t=0 we impose a harmonic trap
           V(i,j)=c1*c2*c3*Exp(-(t-9)**2)*x                 !! offset in x; then for t>0 we move the trap to the centre.
         else
           V(i,j)=0.d0
         endif
...
!! Subroutine to perform a simple, trapezium rule to return the momentum 
      ...
      integer i,j
      double precision xmax,P1,x
      complex*16 psi(0:Nx+1,0:Ny+1), psix(0:Nx+1,0:Ny+1)
     
      P1=0.d0                           !! first set the momentum to zero
      do i=1,Nx                         !! Sum all contributions over the whole grid.
         j=((Ny+1)/2.d0)
         x=-xmax+dble(i-1)*dx
         psix(i,j)=dcmplx((dreal(psi(i+1,j)-
     $     psi(i-1,j))/(2.d0*dx)),
     $     (dimag(psi(i+1,j)-
     $     psi(i-1,j))/(2.d0*dx)))
      P1 = P1+real(dimag(conjg(psi(i,j))*psix(i,j)))
...
******************************************************************************************************
      subroutine get_phase(psi,phase,N)

!! Subroutine to get the phase difference between the end points.

      include 'defins.i'

      integer i,j
      double precision phase,phase1,phase2,N,N1
      complex*16 psi(0:Nx+1,0:Ny+1)

      phase=0.d0                            !! First set N to be zero

                            
       j=((Ny+1)/2.d0)
       N1=Sqrt(real(N))
       phase1=real(datan2(dimag(psi(Nx,j)/N1),dreal(psi(Nx,j)/N1)))   
       phase2=real(datan2(dimag(psi(1,j)/N1),dreal(psi(1,j)/N1)))
       phase=phase1-phase2
...
***********************************************************************
      ...
      subroutine get_X(psi,R,xmax,ymax)
...
!! Subroutine to perform a simple, trapezium rule integration of x*|psi|^2 to return the position.
      ...
      integer i,j
      double precision xmax,R,holder1,x,y,ymax
      complex*16 psi(0:Nx+1,0:Ny+1)
...
******************************************************************************************************
      subroutine get_Force(psi,psi2,Force,xmax)

!! Subroutine to perform a simple, trapezium rule to return the force in the azimuthal direction

      include 'defins.i'

      integer i,j
      double precision xmax,x,force,P1,Pl2,y
      complex*16 psi(0:Nx+1,0:Ny+1), psi2(0:Nx+1,0:Ny+1)
      complex*16 psix(0:Nx+1,0:Ny+1)
      Pl2=0.d0
      P1=0.d0                           !! first set the momentum to zero
      f1=1.0
      do i=1,Nx                         !! Sum all contributions over the whole grid.
     ...
     $     (dimag(psi(i+1,j)-
     $     psi(i-1,j))/(2.d0*dx)))
       P1 = P1+real(dimag(conjg(psi(i,j))*psix(i,j)))*dx 
       ...
       do i=1,Nx                        !! Sum all contributions over the whole grid.
         j=((Ny+1)/2.d0)
         x=-xmax+dble(i-1)*dx
         psix(i,j)=dcmplx((dreal(psi2(i+1,j)-
     $     psi2(i-1,j))/(2.d0*dx)),
     $     (dimag(psi2(i+1,j)-
     $     psi2(i-1,j))/(2.d0*dx)))
       P12 = P12+real(dimag(conjg(psi2(i,j))*psix(i,j)))*dx 
	         
       end do
       

       P12=P12*dx 
       force=(P1-P12)/dt_real
...
!! return it to the main program.
      ...
      do i=1,Nx
        do j=1,Ny
          U(i,j)=1.d0-zabs(psi(i,j))**2-V(i,j)
        end do
      end do
...
!! in the energy integral evaluates via finite difference approximation.
      ...
      do i=1,Nx
        do j=1,Ny
          ...
          E=E+0.5d0*(mdpsisq-1.0d0)**2                    !! Interaction energy 
...
*********************************************************************
      subroutine get_RHS_x(dt,psi,U,RHS_x)
...
!! to contain all terms.     
     ...
     &             (dcmplx(1.d0,0.d0)+
     &                  dt*dcmplx(0.d0,0.5d0*(U(i,j)-1.d0/dx**2)))
     &                                                *psi(i,j)  +
...
**********************************************************************
      subroutine solve_x(dt,RHS_x,U,psi)  
...
***********************************************************************
      subroutine movie(xmax,ymax,iframe,psi,t,P1,Force,N,R)
...
!! intervals, suitable for generating movies of the evolution of density.
      ...
      include          'defins.i'
      integer           i,j,iframe
      double precision  xmax,ymax,Force,R,N,t,P1,N1
      complex*16        psi(0:Nx+1,0:Ny+1)
      ...
      do i=1,Nx
        ...
        do j=1,Ny
          y=-ymax+dble(j-1)*dy
          write(12,'(4e14.6)')real(x),real(P1)/real(N),
     &real(datan2(dimag(psi(i,j)/N1),dreal(psi(i,j)/N1))),
     &real(zabs(psi(i,j))**2)
        end do
      end do
