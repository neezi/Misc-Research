...
*************************************** DEFINITION OF VARIABLES***************************************************
      ...
      double precision    xmax,ymax,force                         !! The half-widths of the box in x and y and force
      double precision    N,N_old,N_diff,R,P1,P1b                  !! Variables for the particle number N, 
...
** PRELIMINARIES / INITIALIZATION OF VARIABLES:
      ...
      do i=0,Nx+1                                              !! Initialize the wavefunction to some initial
	    ...
	    psi2(i,j)=dcmplx(0.d0,0.d0) 
	  else
            x=-xmax+dble(i-1)*dx
	    psi(i,j)=dcmplx(Exp(-x**2/0.1),-2.0*x)                   !! Set that initial wave function
            psi2(i,j)=dcmplx(Exp(-x**2/0.1),-2.0*x)
	  end if                           
...
** REAL TIME OPERATIONS:
        ...
        if(dabs(dreal(dt)).gt.1.d-10)then                       !! If propagating in real time, perform the following tasks:
          ...
          if(itime2.eq.0.or.mod(itime2,5).eq.0)then             !! If the first real time step or every subsequent 200 steps....
            ...
            call get_N(psi,N)   
            call get_force(psi,psi2,force,xmax)
	    call get_p1(psi,P1,xmax) 
	    call get_p1b(psi,P1b,xmax,t)                        !! Determine momenta.
     ...
     &       'P1=',real(R)/real(N)
          ...
          if(itime2.eq.0.or.mod(itime2,3).eq.0)then             !! If the first real time step or every subsequent 500 steps....
            call movie(xmax,ymax,iframe,psi,force,t) 
	    call get_X(psi,R,xmax)
            call get_N(psi,N)
	    write(13,*)real(t),real(R)/real(N)                   !! Call the routine to genenerate movie frames
          end if
        end if
...
!! time-dependent, in which it should be called every real-time step.  
      ...
      integer           i,j,i2,j2
      double precision  V(Nx,Ny),x,y,xmax,ymax,t,V2,V3,V4,Vhold,x2
      parameter         (pi=3.1415926535897932d0)
      ...
      do i=1,Nx                                             !! Loop over x-indices
       ...
       do j=1,Ny                                            !! Loop over y-indices.
        ...
        Vhold=0.d0 		                   
        V(i,j)=-100.0d0*(atan(1.0d0*(x+40.0d0*t - 5.0d0)) + 1.375)          
        do i2=((Ny+1)/2),i                         !! Sum all contributions over the whole grid.
        j2=((Ny+1)/2)
          x2=-xmax+dble(i-1)*dx
	  V2=x2+40.0d0*t - 5.0d0
	  V3=1.0d0/(1.0d0+V2*V2)
	  V4= (200.0d0)*V3
	  Vhold= Vhold-V4*dx        
        end do
        V(i,j)=V(i,j)+Vhold
...
!! Subroutine to perform a simple, trapezium rule to return the canonical momentum 
...
******************************************************************************************************
      subroutine get_p1b(psi,P1b,xmax,t)

!! Subroutine to perform a simple, trapezium rule to return the kinetic momentum

      include 'defins.i'

      integer i,j
      double precision xmax,P1b,x,V2,t,N,holder2
      complex*16 psi(0:Nx+1,0:Ny+1), psix(0:Nx+1,0:Ny+1)
      
      V2=0.d0
      holder2=0.d0
      N=0.d0
      P1b=0.d0                           !! first set the momentum to zero
      do i=1,Nx                         !! Sum all contributions over the whole grid.
         j=((Ny+1)/2)
         x=-xmax+dble(i-1)*dx
         psix(i,j)=dcmplx((dreal(psi(i+1,j)-
     ...
     $     psi(i-1,j))/(2.d0*dx)))
      N=N+real(zabs(psi(i,j))**2) 
      P1b = P1b+real(dimag(conjg(psi(i,j))*psix(i,j)))*dx 
      V2=-0.013333*(atan(1.0d0*(x+150.0d0*t - 8.0d0)) + pi/2.0d0)         
      holder2 = holder2 + N*V2
      end do
...
!! Subroutine to perform a simple, trapezium rule to return the momentum 
      ...
      integer i,j
      double precision xmax,x,force,P1,Pl2
...
!! Subroutine to perform a simple, trapezium rule integration of x*|psi|^2 to return the position.
      ...
      integer i,j
      double precision xmax,R,holder1,x
      complex*16 psi(0:Nx+1,0:Ny+1)
...
***********************************************************************
      subroutine movie(xmax,ymax,iframe,psi,force,t)
...
!! intervals, suitable for generating movies of the evolution of density.
      ...
      include          'defins.i'
      integer           i,j,iframe
      double precision  xmax,ymax,x,t,Pl,force
      complex*16        psi(0:Nx+1,0:Ny+1),psi2(0:Nx+1,0:Ny+1)
     ...
     $     (dimag(psi(i+1,j)-
     $     psi(i-1,j))/(2.d0*dx)))
      Pl= Pl+real(dimag(conjg(psi(i,j))*psi2(i,j)))*dx 
      end do
      Pl=Pl*dx
      do i=1,Nx
        x=-xmax+dble(i-1)*dx
          j=((Ny+1)/2)
          y=-ymax+dble(j-1)*dy
          write(12,'(4e14.6)')real(x),0.2*real(force),  !! making these smaller for viewing convenience
     &real(zabs(psi(i,j))**2),
     &real(atan(1.0d0*(x+40.0d0*t - 5.0d0))+1.375)
